...
| `MAX_FAST_LIMIT` | 200 | fast route の取得上限 |
| `MAX_STANDARD_LIMIT` | 200 | standard route の取得上限 |

### ルート

- **fast**: プランナーが直接 steps を返した場合。1〜2 ステップで完結する軽量クエリ。
- **standard**: 複数ステップの追加検索が必要な場合。ステップごとに結果を蓄積する。
